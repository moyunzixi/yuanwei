/* global window, document, location */
;(function () {
  'use strict'
  const LS = window.LS
  const engine = LS.engine
  const score = LS.score
  const storage = LS.storage
  const characters = LS.characters
  const endings = LS.endings
  const scenes = LS.scenes
  const report = LS.report

  const $ = sel => document.querySelector(sel)
  const $$ = (sel, ctx) => Array.prototype.slice.call((ctx || document).querySelectorAll(sel))

  const BG_MAP = {
    room_night: 'bg_room_night',
    home_evening: 'bg_room_night',
    office: 'bg_office',
    cafe: 'bg_cafe',
    street: 'bg_street',
    archive: 'bg_archive'
  }

  // ---- 运行时状态 ----
  let game = null // { save, node, allLines, cursor, charId, finished, shown }
  let radarDrawn = false

  // ============ 路由 ============
  function parseQuery(hash) {
    const q = {}
    const i = hash.indexOf('?')
    if (i < 0) return q
    hash.slice(i + 1).split('&').forEach(pair => {
      const kv = pair.split('=')
      if (kv[0]) q[decodeURIComponent(kv[0])] = decodeURIComponent(kv[1] || '')
    })
    return q
  }

  const ROUTES = {
    '#/': 'index',
    '#/prologue': 'prologue',
    '#/game': 'game',
    '#/result': 'result',
    '#/history': 'history',
    '#/about': 'about',
    '#/deleted': 'deleted'
  }

  function activate(name) {
    $$('.view').forEach(v => v.classList.remove('active'))
    const v = document.getElementById('view-' + name)
    if (v) v.classList.add('active')
    window.scrollTo(0, 0)
  }

  function router() {
    const hash = location.hash || '#/'
    const path = hash.split('?')[0]
    const name = ROUTES[path] || 'index'
    radarDrawn = false
    activate(name)
    const fns = {
      index: renderIndex, prologue: renderPrologue, game: gameRender,
      result: renderResult, history: renderHistory, about: renderAbout, deleted: renderDeleted
    }
    fns[name]()
  }

  // ============ 工具 ============
  function esc(s) {
    return String(s == null ? '' : s).replace(/[&<>"]/g, c => (
      { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]
    ))
  }

  /* ===== 性别接口 =====
   * 开局选择的玩家性别决定四位对象的性别版本：
   *   玩家为男 → 对象为女（avatarF，文案用"她"）
   *   玩家为女 → 对象为男（avatarM，文案用"他"）
   * 头像随性别切换，正文/选项/结局/报告里的人称代词由 gswap 统一处理。
   */
  function curGender() {
    const s = (game && game.save) || storage.load()
    return (s && s.player && s.player.gender) || 'm'
  }

  function avatarOf(c) {
    if (!c) return ''
    return curGender() === 'f'
      ? (c.avatarM || c.avatarF || c.avatar)
      : (c.avatarF || c.avatar)
  }

  // 她↔他、她们↔他们、前夫↔前妻、女友↔男友、前女友↔前男友、砚姐↔砚哥……
  // 用占位符避免二次替换；泛指词（其他 / 他人）先摘出，最后原样还原
  function gswap(s) {
    const t = String(s == null ? '' : s)
    if (curGender() !== 'f') return t
    return t
      // 第一步：把所有复合词整体摘成唯一占位符（不做性别翻转，避免被单字规则切碎）
      .replace(/前夫/g, '\u0001').replace(/前妻/g, '\u0011')
      .replace(/前女友/g, '\u0007').replace(/前男友/g, '\u0017')
      .replace(/女朋友/g, '\u0008').replace(/男朋友/g, '\u0018')
      .replace(/女友/g, '\u0009').replace(/男友/g, '\u0019')
      .replace(/妻子/g, '\u000A').replace(/丈夫/g, '\u001A')
      .replace(/老婆/g, '\u000B').replace(/老公/g, '\u001B')
      .replace(/砚姐/g, '\u000C').replace(/砚哥/g, '\u001C')
      .replace(/她们/g, '\u0002').replace(/他们/g, '\u0012')
      .replace(/其他/g, '\u0005').replace(/他人/g, '\u0006')
      // 第二步：裸单字 她↔他
      .replace(/她/g, '\u0004').replace(/他/g, '\u0014')
      // 第三步：统一还原占位符（方向已翻转）
      .replace(/\u0001/g, '前妻').replace(/\u0011/g, '前夫')
      .replace(/\u0007/g, '前男友').replace(/\u0017/g, '前女友')
      .replace(/\u0008/g, '男朋友').replace(/\u0018/g, '女朋友')
      .replace(/\u0009/g, '男友').replace(/\u0019/g, '女友')
      .replace(/\u000A/g, '丈夫').replace(/\u001A/g, '妻子')
      .replace(/\u000B/g, '老公').replace(/\u001B/g, '老婆')
      .replace(/\u000C/g, '砚哥').replace(/\u001C/g, '砚姐')
      .replace(/\u0002/g, '他们').replace(/\u0012/g, '她们')
      .replace(/\u0004/g, '他').replace(/\u0014/g, '她')
      .replace(/\u0005/g, '其他').replace(/\u0006/g, '他人')
  }

  function modal(title, body) {
    const mask = document.createElement('div')
    mask.className = 'modal-mask'
    mask.innerHTML =
      '<div class="modal"><h3>' + esc(title) + '</h3><p>' + esc(body) + '</p>' +
      '<button class="btn btn--primary modal__close">知道了</button></div>'
    mask.addEventListener('click', e => {
      if (e.target === mask || e.target.classList.contains('modal__close')) mask.remove()
    })
    document.body.appendChild(mask)
  }

  // ============ 封面 ============
  function renderIndex() {
    const view = $('#view-index')
    const save = storage.load()
    const hasSave = !!(save && save.current && !save.finished)
    const hasPlayed = storage.getHistory && storage.getHistory().length > 0
    let info = ''
    if (hasSave) {
      const dt = new Date(save.createdAt)
      const p2 = n => (n < 10 ? '0' + n : '' + n)
      info = p2(dt.getMonth() + 1) + '/' + p2(dt.getDate()) + ' ' + p2(dt.getHours()) + ':' + p2(dt.getMinutes())
    }
    view.innerHTML =
      '<div class="cover">' +
        '<img class="cover__bg" src="images/bg_archive.jpg" alt="" />' +
        '<div class="cover__mask"></div>' +
        '<div class="cover__content">' +
          '<div class="brand__no">CASE 027</div>' +
          '<div class="brand__name">检茶员</div>' +
          '<div class="brand__sub">第八周 · 互动阅读</div>' +
          '<div class="hr"></div>' +
          '<div class="notice"><p>你的工作很简单：</p><p>进入一段关系，观察里面的人，</p><p>标记可疑对象。</p></div>' +
          '<div class="actions">' +
            '<button class="btn btn--cta" data-act="new">领取新档案</button>' +
            (hasSave ? '<button class="btn" data-act="continue"><div>继续上次</div><div class="btn__sub">' + esc(info) + '</div></button>' : '') +
            '<button class="btn" data-act="history">往期档案</button>' +
            (hasPlayed ? '<button class="btn" data-act="deleted">看看她曾经删除的话</button>' : '') +
            '<button class="btn btn--ghost" data-act="about">关于</button>' +
          '</div>' +
        '</div>' +
      '</div>'

    view.querySelector('[data-act="new"]').addEventListener('click', () => { location.hash = '#/prologue' })
    if (hasSave) {
      view.querySelector('[data-act="continue"]').addEventListener('click', () => { location.hash = '#/game' })
    }
    view.querySelector('[data-act="history"]').addEventListener('click', () => { location.hash = '#/history' })
    const delEnt = view.querySelector('[data-act="deleted"]')
    if (delEnt) delEnt.addEventListener('click', () => { location.hash = '#/deleted' })
    view.querySelector('[data-act="about"]').addEventListener('click', () => { location.hash = '#/about' })
  }

  // ============ 序章 + 开局设置 ============
  function renderPrologue() {
    const view = $('#view-prologue')
    let setup = { gender: 'm', relation: 'plain' }

    view.innerHTML =
      '<div class="prologue__skip" data-act="skip">跳过</div>' +
      '<div class="prologue">' +
        '<div class="prologue__step">SETUP</div>' +
        '<p>在开始之前，先确认你的身份。</p>' +
        '<p>你（玩家）的性别？</p>' +
        '<p class="t-dim t-sm">档案里的四个人会与你的性别对应：你选男，故事里就是她们；你选女，故事里就是他们。立绘与称呼都会跟着变。</p>' +
        '<div class="choices">' +
          '<button class="btn" data-gender="m">男</button>' +
          '<button class="btn" data-gender="f">女</button>' +
        '</div>' +
        '<p style="margin-top:24px">你与 TA 的关系现状？</p>' +
        '<div class="choices">' +
          '<button class="btn" data-rel="passion">热恋中</button>' +
          '<button class="btn" data-rel="plain">平淡稳定</button>' +
          '<button class="btn" data-rel="tired">已经有些疲惫</button>' +
        '</div>' +
        '<div class="prologue__next btn btn--primary" data-act="start">进入档案</div>' +
      '</div>'

    $$('[data-gender]', view).forEach(b => b.addEventListener('click', () => {
      setup.gender = b.getAttribute('data-gender')
      $$('[data-gender]', view).forEach(x => x.style.borderColor = '')
      b.style.borderColor = 'var(--accent)'
    }))
    $$('[data-rel]', view).forEach(b => b.addEventListener('click', () => {
      setup.relation = b.getAttribute('data-rel')
      $$('[data-rel]', view).forEach(x => x.style.borderColor = '')
      b.style.borderColor = 'var(--accent)'
    }))
    view.querySelector('[data-act="skip"]').addEventListener('click', () => {
      storage.clearSave()
      storage.save(engine.createSave({ gender: 'm', relation: 'plain' }))
      location.hash = '#/game'
    })
    view.querySelector('[data-act="start"]').addEventListener('click', () => {
      storage.clearSave()
      const save = engine.createSave(setup)
      storage.save(save)
      location.hash = '#/game'
    })
  }

  // ============ 游戏 ============
  function updateGameEnv() {
    const view = $('#view-game')
    const node = game.node
    const bg = BG_MAP[node.bg] || 'bg_room_night'
    const bgEl = view.querySelector('.game__bg')
    if (bgEl) bgEl.src = 'images/' + bg + '.jpg'
    view.querySelector('.game__top .time').textContent = node.time || ''
  }

  function decorate(raw) {
    const who = raw.who || 'narrator'
    const text = gswap(raw.text)
    if (who === 'narrator') return { isNarrator: true, text: text }
    if (who === 'system') return { isSystem: true, text: text }
    if (who === 'me') return { isMe: true, text: text }
    const c = characters[who]
    return {
      isNarrator: false,
      charId: who,
      name: c ? c.name : who,
      avatar: avatarOf(c) || who,
      color: c ? c.color : '#5B87A8',
      text: text
    }
  }

  // 对话流：人物小图 + 气泡；旁白无头像，系统为档案行
  function renderStream() {
    const view = $('#view-game')
    const stream = view.querySelector('.stream__inner')
    stream.innerHTML = game.shown.map(it => {
      if (it.isSystem) {
        return '<div class="msg msg--sys"><span>' + esc(it.text) + '</span></div>'
      }
      if (it.isNarrator) {
        return '<div class="msg msg--narr"><div class="msg__text">' + esc(it.text) + '</div></div>'
      }
      if (it.isMe) {
        return '<div class="msg msg--me">' +
          '<div class="msg__avatar msg__avatar--me">我</div>' +
          '<div class="msg__body"><div class="msg__name">你</div>' +
          '<div class="msg__text">' + esc(it.text) + '</div></div></div>'
      }
      return '<div class="msg">' +
        '<img class="msg__avatar" src="images/' + esc(it.avatar) + '.jpg" alt="" />' +
        '<div class="msg__body">' +
          '<div class="msg__name" style="color:' + it.color + '">' + esc(it.name) + '</div>' +
          '<div class="msg__text">' + esc(it.text) + '</div>' +
        '</div></div>'
    }).join('') + '<div class="pad"></div>'
    const box = stream.parentElement
    box.scrollTop = box.scrollHeight
  }

  function renderChoices() {
    const view = $('#view-game')
    const box = view.querySelector('.choices')
    const list = engine.visibleChoices(game.save, game.node)
    if (game.finished && list.length) {
      box.innerHTML = list.map(x =>
        '<button class="btn choice" data-i="' + x.index + '">' + esc(gswap(x.choice.text)) + '</button>'
      ).join('')
      $$('[data-i]', box).forEach(b => b.addEventListener('click', () => {
        if (b.classList.contains('is-picked')) return
        b.classList.add('is-picked') // 选中：放大 + 变色，略微停顿后再推进
        const idx = Number(b.getAttribute('data-i'))
        setTimeout(() => {
          game.save = engine.choose(game.save, idx)
          storage.save(game.save)
          gameRender()
        }, 260)
      }))
    } else {
      box.innerHTML = ''
    }
    const hasVis = (game.node && game.node.choices) ? engine.visibleChoices(game.save, game.node).length > 0 : false
    const needContinue = game.finished && !hasVis && !!(game.node && game.node.next)
    view.querySelector('.hint').style.display = (game.finished && !needContinue) ? 'none' : 'block'
    // 选项框出现后把对话区粘到底部，避免最后几行被选项框压住
    const sb = view.querySelector('.game__stream')
    if (sb) requestAnimationFrame(() => stickBottom(sb))
  }

  // 尊重用户向上回看：仅当接近底部时才自动滚到底
  function stickBottom(box) {
    if (!box) return
    const nearBottom = box.scrollHeight - box.scrollTop - box.clientHeight < 90
    if (nearBottom) box.scrollTop = box.scrollHeight
  }

  function step() {
    if (game.cursor >= game.allLines.length) { revealChoices(); return }
    const raw = game.allLines[game.cursor++]
    const item = decorate(raw)
    game.shown.push(item)
    renderStream()
    if (game.cursor >= game.allLines.length) revealChoices()
  }

  function revealChoices() {
    game.finished = true
    renderChoices()
  }

  // 点击「继续」：推进一行；或（叙事续节点）前进到下一节点
  function advance() {
    if (game.finished) {
      const vis = (game.node && game.node.choices) ? engine.visibleChoices(game.save, game.node) : []
      if (vis.length) return
      if (game.node && game.node.next) {
        game.save = engine.advance(game.save)
        storage.save(game.save)
        gameRender()
      }
      return
    }
    step()
  }

  function gameRender() {
    const view = $('#view-game')
    if (!view.querySelector('.game__bg')) {
      view.innerHTML =
        '<div class="game">' +
          '<img class="game__bg" src="" alt="" />' +
          '<div class="game__mask"></div>' +
          '<div class="game__top"><span class="time t-mono t-dim t-sm"></span>' +
            '<span class="quit">离开</span></div>' +
          '<div class="game__stream"><div class="stream__inner"></div></div>' +
          '<div class="game__bottom">' +
            '<div class="hint">轻触继续</div>' +
            '<div class="choices"></div>' +
          '</div>' +
        '</div>'
      view.querySelector('.quit').addEventListener('click', () => {
        if (confirm('离开这段关系？当前进度会保留。')) location.hash = '#/'
      })
      // 全屏任意处点击「继续」（选项 / 离开 / 顶部栏等索引键除外）
      view.querySelector('.game').addEventListener('click', (e) => {
        if (e.target.closest('.choice, .quit, .game__top')) return
        advance()
      })
    }

    let save = storage.load()
    if (!save || !save.current || save.finished) {
      // 没有有效存档：回封面
      if (!save || !save.current) { location.hash = '#/'; return }
    }
    game = game || {}
    game.save = save

    // 自动推进无文本/无选项的 branch 节点
    let guard = 0
    while (guard++ < 6) {
      const node = engine.currentNode(game.save)
      if (!node) break
      const hasLines = node.lines && node.lines.length > 0
      const hasChoices = node.choices && node.choices.length > 0
      if (hasLines || hasChoices) break
      const next = engine.advance(game.save)
      if (!next || next.current === game.save.current) break
      game.save = next
    }
    storage.persist(game.save)
    if (engine.isFinished(game.save)) { goResult(game.save.current); return }

    game.node = engine.currentNode(game.save)
    game.allLines = game.node.lines || []
    game.cursor = 0
    game.shown = []
    game.finished = false
    updateGameEnv()
    const scrollBox = view.querySelector('.game__stream')
    if (scrollBox) scrollBox.scrollTop = 0
    renderStream()
    renderChoices()
    step()
  }

  function goResult(endingId) {
    const save = game ? game.save : storage.load()
    if (!save) { location.hash = '#/'; return }
    const rd = score.buildReport(save, endingId)
    storage.addHistory({
      index: rd.index, rank: rd.rankName, ending: endingId,
      endingName: endings[endingId] ? endings[endingId].name : '',
      quadrant: rd.quadrantKey
    })
    const finished = Object.assign({}, save, { finished: true })
    storage.save(finished)
    location.hash = '#/result?ending=' + endingId
  }

  // ============ 结果 ============
  function renderResult() {
    const view = $('#view-result')
    const q = parseQuery(location.hash)
    const endingId = q.ending || 'e_almost'
    const save = storage.load()
    if (!save) { location.hash = '#/'; return }
    const rd = score.buildReport(save, endingId)
    const ending = endings[endingId]
    const notFate = report.notFate

    const dims = rd.dimensions.map(d =>
      '<div class="dim" data-k="' + d.key + '">' +
        '<div class="dim__label">' + esc(d.label) + '</div>' +
        '<div class="dim__bar"><div class="dim__fill" style="width:' + d.value + '%"></div></div>' +
        '<div class="dim__val">' + d.value + '</div>' +
      '</div>'
    ).join('')

    const replay = rd.replay.map(r =>
      '<div class="replay"><div class="replay__label">' + esc(r.time + r.label) + '</div>' +
      '<div class="replay__text">' + esc(gswap(r.text)) + '</div></div>'
    ).join('')

    const patterns = rd.patterns.map(p => '<div class="pat">· ' + esc(gswap(p.line)) + '</div>').join('')

    const matches = rd.matches.map(m =>
      '<div class="match"><div class="match__name">' + esc(m.name) + '</div>' +
      '<div class="match__bar"><div class="match__fill" style="width:' + m.sim + '%"></div></div>' +
      '<div class="match__val">' + m.sim + '%</div></div>'
    ).join('')

    const endingText = ending ? ending.text.map(t => '<div class="ending-line">' + esc(gswap(t)) + '</div>').join('') : ''

    view.innerHTML =
      '<div class="reveal" data-stage="0">' +
        '<div class="reveal__no">CASE 027</div>' +
        '<div class="reveal__line fade-in">八周，结束了。</div>' +
        '<div class="reveal__line fade-in">这一回，被检的不只是他们。</div>' +
        '<div class="reveal__you">你也在这份档案里。</div>' +
        '<div class="reveal__hint">轻触，继续</div>' +
      '</div>' +
      '<div class="bridge" data-stage="1" style="display:none">' +
        '<div class="bridge__card">' +
          '<div class="bridge__no">RELATIONSHIP ANALYSIS</div>' +
          '<div class="bridge__title">这是一份只属于你的档案</div>' +
          '<p class="bridge__desc">它不会评判你。它会把你这八周的每一次选择，原原本本摊开——包括那些你以为没人看见的。</p>' +
          '<button class="btn btn--cta bridge__open" data-act="open">开启我的档案</button>' +
          '<div class="bridge__sub t-dim t-sm">点击后，报告将基于你刚才的选择实时生成。</div>' +
        '</div>' +
      '</div>' +
      '<div class="report" data-stage="2" style="display:none">' +
        '<div class="sec"><div class="sec__no">RELATIONSHIP ANALYSIS</div>' +
          '<div class="index-row"><div class="index-num">' + rd.index + '</div>' +
          '<div class="index-meta"><div class="rank">' + esc(rd.rankName) + '</div>' +
          '<div class="t-dim t-sm">' + esc(gswap(rd.rankLine)) + '</div></div></div>' +
          '<div class="t-dim t-sm mt">关系边界指数 · 共记录 ' + rd.stats.totalChoices + ' 次选择</div>' +
        '</div>' +
        '<div class="sec"><div class="radar-wrap"><canvas id="radar" class="radar"></canvas></div></div>' +
        '<div class="sec"><div class="sec__title">各项明细</div>' + dims +
          '<div class="t-dim t-sm mt">（点一下维度名，看看它在测什么）</div></div>' +
        (rd.replay.length ? '<div class="sec"><div class="sec__title">来，回顾一下你的高光时刻</div>' + replay + '</div>' : '') +
        (rd.patterns.length ? '<div class="sec"><div class="sec__title">你的行为路线</div>' + patterns + '</div>' : '') +
        (rd.rationalizationLine ? '<div class="sec"><div class="sec__title">你对自己说过的话</div><div class="pat">' + esc(gswap(rd.rationalizationLine)) + '</div></div>' : '') +
        '<div class="sec"><div class="sec__title">你更像剧本里的谁</div>' +
          '<div class="quad"><div class="quad__name">' + esc(gswap(rd.quadrant.headline)) + '</div>' +
          '<div class="quad__line">' + esc(gswap(rd.quadrant.line)) + '</div>' +
          '<div class="t-dim t-sm mt">' + esc(gswap(rd.quadrant.diff)) + '</div>' +
          '<div class="t-dim t-sm mt">' + esc(notFate) + '</div>' +
          '<button class="btn btn--ghost mt" data-act="insight">' + esc(gswap('她到底怎么回事？')) + '</button></div>' +
          '<div class="hr"></div>' + matches +
        '</div>' +
        (ending ? '<div class="sec"><div class="sec__no">' + esc(ending.subtitle) + '</div>' +
          '<div class="sec__title">' + esc(ending.name) + '</div>' + endingText +
          '<div class="cost"><div class="lbl">代价</div><div class="val t-dim">' + esc(gswap(ending.cost)) + '</div></div>' +
          '<div class="pat mt">' + esc(gswap(ending.note)) + '</div></div>' : '') +
        '<div class="sec closing">' + report.closing.map(c => '<p>' + esc(gswap(c)) + '</p>').join('') + '</div>' +
        '<div class="disclaimer">' + esc(report.disclaimer) + '</div>' +
        '<div class="foot">' +
          '<button class="btn btn--ghost" data-act="deleted">看看她曾经删除的话</button>' +
          '<button class="btn btn--primary" data-act="home">回到档案柜</button>' +
        '</div>' +
      '</div>'

    // 阶段一：轻触继续
    view.querySelector('.reveal').addEventListener('click', () => {
      view.querySelector('.reveal').style.display = 'none'
      view.querySelector('.bridge').style.display = 'flex'
    })
    // 阶段二：引导页开启报告
    view.querySelector('[data-act="open"]').addEventListener('click', () => {
      view.querySelector('.bridge').style.display = 'none'
      view.querySelector('.report').style.display = 'block'
      drawRadar(rd.dimensions)
    })
    // 维度说明
    $$('.dim', view).forEach(d => d.addEventListener('click', () => {
      const key = d.getAttribute('data-k')
      const dim = rd.dimensions.find(x => x.key === key)
      if (dim && dim.science) modal(dim.science.title, dim.science.body)
    }))
    view.querySelector('[data-act="insight"]').addEventListener('click', () => {
      const c = characters[rd.quadrant.character]
      if (c) modal(gswap(c.misread), gswap(c.insight))
    })
    view.querySelector('[data-act="home"]').addEventListener('click', () => { location.hash = '#/' })
    const delBtn = view.querySelector('[data-act="deleted"]')
    if (delBtn) delBtn.addEventListener('click', () => { location.hash = '#/deleted' })
  }

  // ============ 隐藏模块：看看她曾经删除的话 ============
  function getDeletedSeen() {
    try { return JSON.parse(localStorage.getItem('ls_deleted_seen_v1') || '[]') } catch (e) { return [] }
  }
  function markDeletedSeen(id) {
    const a = getDeletedSeen()
    if (a.indexOf(id) < 0) {
      a.push(id)
      try { localStorage.setItem('ls_deleted_seen_v1', JSON.stringify(a)) } catch (e) {}
    }
  }
  function draftLine(s) {
    switch (s.k) {
      case 'input': return '<div class="draft draft--input">' + esc(gswap(s.text)) + '</div>'
      case 'pause': return '<div class="draft draft--pause">停顿。</div>'
      case 'stay': return '<div class="draft draft--pause">' + esc(gswap(s.text || '停留')) + '</div>'
      case 'delete': return '<div class="draft draft--act">— 删除 —</div>'
      case 'modify': return '<div class="draft draft--act">— 再修改 —</div>'
      case 'send': return '<div class="draft draft--send">' + esc(gswap(s.text)) + '</div>'
      case 'nosend': return '<div class="draft draft--act">— 最后什么都没发 —</div>'
      default: return ''
    }
  }

  function renderDeleted() {
    const view = $('#view-deleted')
    const save = storage.load()
    const data = LS.shadows
    const expr = LS.expr
    const records = (data.records || []).map(r => {
      const locked = r.cond ? !expr.evaluate(r.cond, save) : false
      return Object.assign({}, r, { locked: !!locked })
    })
    const seen = getDeletedSeen()
    const unlockedCount = records.filter(r => !r.locked).length
    const su0 = records.filter(r => !r.locked && seen.indexOf(r.id) >= 0).length

    const lineNarr = (t) => '<div class="msg msg--narr"><div class="msg__text">' + esc(gswap(t)) + '</div></div>'
    const lineSys = (t) => '<div class="msg msg--sys"><span>' + esc(gswap(t)) + '</span></div>'

    const introHtml = (data.intro || []).map(line =>
      line.who === 'system' ? lineSys(line.text) : lineNarr(line.text)
    ).join('')

    const cardsHtml = records.map(r => {
      const c = characters[r.char]
      if (r.locked) {
        return '<div class="dcard dcard--lock">' +
          '<div class="dcard__head"><div class="dcard__avatar dcard__avatar--lock">?</div>' +
          '<div><div class="dcard__name">？？？</div><div class="dcard__meta">未解锁 · 取决于你的选择</div></div></div>' +
        '</div>'
      }
      const avatar = avatarOf(c) || r.char
      const name = c ? gswap(c.name) : gswap(r.char)
      const sentHtml = (r.sent || []).map(t => '<div class="draft draft--send">' + esc(gswap(t)) + '</div>').join('')
      const narrHtml = (r.narrator || []).map(t => '<div class="dcard__narr">' + esc(gswap(t)) + '</div>').join('')
      const quoteHtml = r.quote ? '<div class="draft draft--send">' + esc(gswap(r.quote)) + '</div>' : ''
      const afterHtml = (r.after || []).map(t => '<div class="dcard__narr">' + esc(gswap(t)) + '</div>').join('')
      return '<div class="dcard" data-id="' + r.id + '">' +
        '<div class="dcard__head">' +
          '<img class="dcard__avatar" src="images/' + esc(avatar) + '.jpg" alt="" />' +
          '<div><div class="dcard__name">' + esc(name) + '</div>' +
          '<div class="dcard__meta">' + esc(gswap(r.time)) + ' · ' + esc(gswap(r.scene)) + '</div></div>' +
          '<div class="dcard__chev">▾</div>' +
        '</div>' +
        '<div class="dcard__sent"><div class="dcard__sent-label">她当时发出的是：</div>' + sentHtml + '</div>' +
        '<div class="dcard__edit" style="display:none">' +
          '<div class="dcard__edit-label">「对方曾编辑过这条消息。」</div>' +
          (r.steps || []).map((s, i) => '<div class="draft-wrap" style="animation-delay:' + (i * 0.16) + 's">' + draftLine(s) + '</div>').join('') +
          narrHtml + quoteHtml + afterHtml +
          '<div class="dcard__mark t-dim t-sm">以上为未发送草稿</div>' +
        '</div>' +
      '</div>'
    }).join('')

    const finalHtml = data.final ? (
      '<div class="dfinal">' +
        '<div class="dfinal__head"><div class="dfinal__title">' + esc(gswap(data.final.title)) + '</div>' +
        '<div class="dfinal__time">' + esc(gswap(data.final.time)) + '</div></div>' +
        (data.final.lines || []).map(l => {
          if (l.who === 'system') return lineSys(l.text)
          if (l.who === 'pause') return '<div class="draft draft--pause">停顿。</div>'
          if (l.who === 'quote') return '<div class="dfinal__quote">' + esc(gswap(l.text)) + '</div>'
          return lineNarr(l.text)
        }).join('') +
      '</div>'
    ) : ''

    const allSeen = unlockedCount > 0 && su0 >= unlockedCount
    const progressTxt = '收集完成度：' + su0 + ' / ' + unlockedCount + '（共 ' + records.length + ' 条）'
    const seenHtml = allSeen
      ? (data.outro.allDone || []).map(l => l.who === 'system' ? lineSys(l.text) : lineNarr(l.text)).join('')
      : ''

    view.innerHTML =
      '<div class="deleted">' +
        '<div class="deleted__top"><span class="time t-mono t-dim t-sm">UNSENT · 未发送</span>' +
          '<span class="quit" data-act="home">返回</span></div>' +
        '<div class="deleted__inner">' +
          '<div class="deleted__head">' +
            '<div class="deleted__title">' + esc(gswap(data.title)) + '</div>' +
            '<div class="deleted__sub">' + (data.subtitle || []).map(t => '<div>' + esc(gswap(t)) + '</div>').join('') + '</div>' +
          '</div>' +
          '<div class="deleted__intro">' + introHtml + '</div>' +
          lineSys('已恢复 ' + unlockedCount + ' 条未发送内容。') +
          (data.systemIntro || []).map(t => lineSys(t)).join('') +
          '<div class="deleted__list">' + cardsHtml + '</div>' +
          finalHtml +
          '<div class="deleted__outro">' +
            lineSys('「' + data.title + '」' + progressTxt) +
            seenHtml +
            '<div class="actions">' +
              (allSeen ? '<button class="btn" data-act="replay">再看一遍</button>' : '') +
              '<button class="btn btn--primary" data-act="home">退出</button>' +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>'

    function replayAll() {
      const inner = view.querySelector('.deleted__inner')
      if (inner) inner.scrollIntoView({ behavior: 'smooth', block: 'start' })
      const first = view.querySelector('.dcard[data-id="d1"]')
      if (first) {
        const edit = first.querySelector('.dcard__edit')
        if (edit.style.display === 'none') { edit.style.display = 'block'; first.classList.add('is-open'); markDeletedSeen('d1') }
        let rep = first.querySelector('.dcard__replay')
        if (!rep) {
          rep = document.createElement('div')
          rep.className = 'dcard__replay'
          rep.innerHTML = (data.outro.replay || []).map(l =>
            l.who === 'quote' ? '<div class="draft draft--send">' + esc(gswap(l.text)) + '</div>' : '<div class="dcard__narr">' + esc(gswap(l.text)) + '</div>'
          ).join('')
          edit.appendChild(rep)
        }
      }
    }

    $$('.dcard', view).forEach(card => {
      if (card.classList.contains('dcard--lock')) return
      card.addEventListener('click', () => {
        const edit = card.querySelector('.dcard__edit')
        if (edit.style.display !== 'none') { edit.style.display = 'none'; card.classList.remove('is-open'); return }
        edit.style.display = 'block'
        card.classList.add('is-open')
        markDeletedSeen(card.getAttribute('data-id'))
        const seen2 = getDeletedSeen()
        const su = records.filter(r => !r.locked && seen2.indexOf(r.id) >= 0).length
        const prog = view.querySelector('.deleted__outro .msg--sys span')
        if (prog) prog.textContent = '「' + data.title + '」收集完成度：' + su + ' / ' + unlockedCount + '（共 ' + records.length + ' 条）'
        if (su >= unlockedCount && !view.querySelector('.deleted__alldone')) {
          const wrap = document.createElement('div')
          wrap.className = 'deleted__alldone'
          wrap.innerHTML = (data.outro.allDone || []).map(l =>
            l.who === 'system' ? lineSys(l.text) : lineNarr(l.text)
          ).join('') + '<div class="actions"><button class="btn" data-act="replay">再看一遍</button></div>'
          view.querySelector('.deleted__outro').appendChild(wrap)
          const rb = wrap.querySelector('[data-act="replay"]')
          if (rb) rb.addEventListener('click', replayAll)
        }
      })
    })

    const replayBtn = view.querySelector('[data-act="replay"]')
    if (replayBtn) replayBtn.addEventListener('click', replayAll)
    view.querySelector('[data-act="home"]').addEventListener('click', () => { location.hash = '#/' })
  }

  function drawRadar(dimensions) {
    if (radarDrawn) return
    const canvas = $('#radar')
    if (!canvas) return
    const dpr = window.devicePixelRatio || 1
    const rect = canvas.getBoundingClientRect()
    const size = Math.max(240, rect.width)
    canvas.width = size * dpr
    canvas.height = size * dpr
    const ctx = canvas.getContext('2d')
    ctx.scale(dpr, dpr)
    ctx.clearRect(0, 0, size, size)

    const cx = size / 2, cy = size / 2, R = size / 2 - 30
    const n = dimensions.length
    const point = (i, r) => {
      const ang = -Math.PI / 2 + (i * 2 * Math.PI) / n
      return [cx + r * Math.cos(ang), cy + r * Math.sin(ang)]
    }

    for (let ring = 1; ring <= 4; ring++) {
      ctx.beginPath()
      for (let i = 0; i < n; i++) {
        const p = point(i, (R * ring) / 4)
        if (i === 0) ctx.moveTo(p[0], p[1]); else ctx.lineTo(p[0], p[1])
      }
      ctx.closePath()
      ctx.strokeStyle = 'rgba(34,40,50,0.12)'
      ctx.lineWidth = 1
      ctx.stroke()
    }
    for (let i = 0; i < n; i++) {
      const p = point(i, R)
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(p[0], p[1])
      ctx.strokeStyle = 'rgba(34,40,50,0.12)'; ctx.stroke()
    }
    ctx.beginPath()
    for (let i = 0; i < n; i++) {
      const p = point(i, (R * dimensions[i].value) / 100)
      if (i === 0) ctx.moveTo(p[0], p[1]); else ctx.lineTo(p[0], p[1])
    }
    ctx.closePath()
    ctx.fillStyle = 'rgba(200,80,60,0.22)'
    ctx.fill()
    ctx.strokeStyle = '#C8503C'
    ctx.lineWidth = 2
    ctx.stroke()
    for (let i = 0; i < n; i++) {
      const p = point(i, (R * dimensions[i].value) / 100)
      ctx.beginPath(); ctx.arc(p[0], p[1], 3, 0, Math.PI * 2); ctx.fillStyle = '#C8503C'; ctx.fill()
    }
    ctx.fillStyle = 'rgba(91,102,114,0.95)'
    ctx.font = '12px sans-serif'
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
    for (let i = 0; i < n; i++) {
      const p = point(i, R + 16)
      ctx.fillText(dimensions[i].label, p[0], p[1])
    }
    radarDrawn = true
  }

  // ============ 历史 ============
  function renderHistory() {
    const view = $('#view-history')
    const list = storage.getHistory()
    const QUAD = { secure: '安全型', preoccupied: '痴迷型', dismissing: '疏离型', fearful: '恐惧型' }
    const fmt = ts => {
      const d = new Date(ts)
      const p = n => (n < 10 ? '0' + n : '' + n)
      return p(d.getMonth() + 1) + '/' + p(d.getDate()) + ' ' + p(d.getHours()) + ':' + p(d.getMinutes())
    }
    const cards = list.length ? list.map(item =>
      '<div class="card"><div class="no">CASE · ' + fmt(item.createdAt) + '</div>' +
      '<div class="row"><div class="idx">' + item.index + '</div><div class="card__meta">' +
      '<div class="rank">' + esc(item.rank) + '</div>' +
      '<div class="t-dim t-sm">' + esc(item.endingName) + ' · ' + (QUAD[item.quadrant] || '') + '</div></div></div></div>'
    ).join('') : '<div class="empty"><div>还没有任何档案。</div><div class="t-dim t-sm mt">完成一次《第八周》，这里会留下记录。</div></div>'

    view.innerHTML =
      '<div class="page-pad"><div class="head"><div class="t-mono">ARCHIVE</div><div class="title">往期档案</div></div>' +
      cards +
      (list.length ? '<div class="foot"><button class="btn btn--ghost" data-act="clear">清除全部记录</button></div>' : '') +
      '<div style="margin-top:24px"><button class="btn" data-act="home">返回</button></div></div>'

    if (list.length) {
      view.querySelector('[data-act="clear"]').addEventListener('click', () => {
        if (confirm('清除往期档案？所有历史记录会被删除，且无法恢复。')) {
          storage.clearHistory(); renderHistory()
        }
      })
    }
    view.querySelector('[data-act="home"]').addEventListener('click', () => { location.hash = '#/' })
  }

  // ============ 关于 ============
  function renderAbout() {
    const view = $('#view-about')
    const refs = report.refs.map(g =>
      '<div class="ref"><div class="ref__group">' + esc(g.group) + '</div>' +
      g.items.map(i => '<div class="ref__item">' + esc(i) + '</div>').join('') + '</div>'
    ).join('')
    view.innerHTML =
      '<div class="page-pad"><div class="head"><div class="t-mono">ABOUT</div><div class="title">关于</div></div>' +
      '<div class="sec"><div class="sec__title">这是什么</div>' +
        '<p class="p">《检茶员》是一款完全本地运行的互动阅读作品。你进入一段虚构关系，做出选择，故事根据你的选择推进。</p>' +
        '<p class="p">全部内容在设备本地运算，不上传任何数据，无需网络，没有账号。</p></div>' +
      '<div class="sec warn-box"><div class="sec__title">重要说明</div>' +
        '<p class="p">本作品是<strong class="strong">娱乐性互动推演</strong>，不是心理测评，不构成任何心理学结论，也不能用于判断真实关系。</p>' +
        '<p class="p">所有人物、情节均为虚构，与现实中的任何人无关。人物的行为逻辑参考了公开发表的心理学研究，但游戏中的测量方式并非标准化量表。</p></div>' +
      '<div class="sec"><div class="sec__title">参考的研究方向</div>' + refs +
        '<div class="t-dim t-sm mt">以上仅为本作品的设计参考，不代表这些研究支持或参与本作品的任何结论。</div></div>' +
      '<div class="sec"><div class="sec__title">数据</div><p class="p t-dim t-sm">进度与历史记录保存在本机浏览器存储中。清除站点数据会一并删除。</p></div>' +
      '<div style="margin-top:20px"><button class="btn" data-act="home">返回</button></div></div>'
    view.querySelector('[data-act="home"]').addEventListener('click', () => { location.hash = '#/' })
  }

  // ============ 启动 ============
  storage.init()
  window.addEventListener('hashchange', router)
  if (!location.hash) location.hash = '#/'
  router()
})()
